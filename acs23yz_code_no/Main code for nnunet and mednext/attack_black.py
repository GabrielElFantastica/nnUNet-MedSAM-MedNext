...
def pgd_attack(model, images, boxes_np, labels, eps=0.2, alpha=0.01, iters=10):
...
def zero_attack(model, images, boxes_np, labels):
    images = images.to(device)
    # boxes_np = torch.tensor(boxes_np).to(device)
    labels = labels.clone().detach().float().to(device)

    prediction = model(images, boxes_np)

    return prediction
    
all4_file_path = '/mnt/fastdata/acs23yz/MedSAM/data/npy/black_Brain/'
...
for epsilon in epsilon_list:
    ...
    for step, (image, gt2D, boxes, names_temp) in enumerate(tqdm(train_dataloader)):
        ...
        if epsilon == 0:
            prediction = zero_attack(medsam_model, images=image, boxes_np=boxes_np, labels=gt2D)
            for idx in range(image.shape[0]):
                save_tensor_as_npy(prediction[idx], join(all4_file_path, 'pgd', f'eps={epsilon}', names_temp[idx]))
                save_tensor_as_npy(prediction[idx], join(all4_file_path, 'fgsm', f'eps={epsilon}', names_temp[idx]))
