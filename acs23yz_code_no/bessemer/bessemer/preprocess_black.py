...
import argparse
parser = argparse.ArgumentParser(description='Black attack')

parser.add_argument('-i', type=int, required=True,
                    help='Choose epsilon for black attack')

args = parser.parse_args()

print(f'Start to preprocess epsilon {args.i / 10}')

nii_path = f"/mnt/fastdata/acs23yz/nnUNetFrame/DATASET/nnUNet_raw/nnUNet_raw_data/Dataset001_BrainTumour/pgd/imagesTs_pgd_{args.i}/stacked_image/"  # path to the nii images
gt_path = "/mnt/fastdata/acs23yz/MedSAM/data/Task01_BrainTumour/labelsTs/"  # path to the ground truth
npy_path = "/mnt/fastdata/acs23yz/MedSAM/data/npy/" + prefix[:-1] + f'/pgd_{args.i}'
...
for name in tqdm(names[:40]):  # use the remaining 10 cases for validation
    ...
    if len(z_index) > 0:
        ...
        # Special setting!
        print(f'Now Data shape is {image_data.shape}')
        ...
        for i in range(img_roi.shape[1]):
            img_c_channels = []
            for channel in range(img_roi.shape[0]):
                img_i = img_roi[channel, i, :, :]
                ...
                img_c_channels.append(resize_img_skimg_01)
            
            # Stack all channels
            resize_img_skimg_01 = np.stack(img_c_channels, axis=0)
            ...
            assert resize_img_skimg_01.shape[1:3] == resize_gt_skimg.shape
            np.save(
                join(
                    npy_path,
                    "imgs",
